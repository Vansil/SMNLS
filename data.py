...
class VuaPosDataset(Dataset):
    def __init__(self, set_name="train"):
        super(VuaPosDataset, self).__init__()

        self.id_to_label = ['DET', 'PROPN', 'NUM', 'PRON', 'X', 'ADV', 'VERB', 'SYM', 'PUNCT', 'CCONJ', 'INTJ', 'PART', 'NOUN', 'ADJ', 'ADP']
        self.label_to_id = self.label_to_id = {self.id_to_label[i]: i for i in range(len(self.id_to_label))}

        with open(os.path.join("data", "vua-sequence", set_name + ".csv"), "r", encoding="ISO-8859-1") as f:
            reader = DictReader(f, fieldnames=["txt_id", "sen_ix", "sentence", "label_seq", "pos_seq", "labeled_sentence", "genre"])
            self._data = [(item['sentence'].split(),
                [self.label_to_id[label] for label in re.findall(r"'(.*?)'",item['pos_seq'])]
